...
class TPN(nn.Module):
    ...
    :parameter
        ...
        n_classes: linear classifier output
        channels_size: conv1d hidden and output channel size.
            default: [32, 64, 96]
            out_channels: 96
        kernel_list: three conv1d layer kernel size
        backbone: if or not use and report linear classifier features
    """
    def __init__(self, in_channels, n_classes, channels_size=None, kernel_list=None, backbone=True):
        super().__init__()
        if kernel_list is None:
            kernel_list = [24, 16, 8]
        if channels_size is None:
