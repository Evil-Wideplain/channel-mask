...
from data.data_preprocess_utils import get_sample_weights, opp_sliding_window, normalize, train_test_val_split, DataDir, \
    train_test_val_fine_split
...
def sep_user_device_gt():
    ...
    dataDir = DataDir + '/HHAR/'
    fileInList = os.listdir(dataDir)

    saveDir = DataDir + '/HHAR/avtivity_data_separated/'
...
def combine_acc_gyr():
    dataDir = DataDir + '/HHAR/avtivity_data_separated/'
    saveDir = DataDir + '/HHAR/avtivity_data_acc_gyr/'
    if not os.path.exists(saveDir):
        os.mkdir(saveDir)
    user_list = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i']
    gt_list = ['bike', 'sit', 'stand', 'walk', 'stairsup', 'stairsdown']  # exclude 'null'
    watch_device = ['gear_1', 'gear_2', 'lgwatch_1', 'lgwatch_2']
    phone_device = ['nexus4_1', 'nexus4_2', 's3_1', 's3_2', 's3mini_1',
                    's3mini_2']  # exclude'samsungold_1', 'samsungold_2'
...
def interpolate():
    dataDir = DataDir + '/HHAR/avtivity_data_acc_gyr/'
    saveDir = DataDir + '/HHAR/avtivity_data_acc_gyr_interp/'
    if not os.path.exists(saveDir):
        os.mkdir(saveDir)
    user_list = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i']
    gt_list = ['bike', 'sit', 'stand', 'walk', 'stairsup', 'stairsdown']  # exclude 'null'
    ...
    for user in user_list:
        for i, device in enumerate(device_list):
            ...
            for gt in gt_list:
                ...
                print(endTime)
                InterpTime = np.linspace(curTimeList[0], endTime,
                                         n_samples * resample_ratio).squeeze()  # TODO n_samples*1 decides downsampling
...
def split_train_test_subject(train_user, test_user, device, SLIDING_WINDOW_LEN=100, SLIDING_WINDOW_STEP=50):
    ...
    dataDir = DataDir + '/HHAR/avtivity_data_acc_gyr_interp/'
...
def load_domain_data(device, args, SLIDING_WINDOW_LEN=100, SLIDING_WINDOW_STEP=50):
    # device: 'Watch' or 'Phones'
    dataDir = DataDir + '/HHAR/avtivity_data_acc_gyr_interp/'
    preprocess_Dir = DataDir + '/HHAR/hhar_processed_' + device + '.data'
    if os.path.isfile(preprocess_Dir):
        ...
        f.close()
    return x_sw, y_sw, d_sw


def split_train_test(device, args, SLIDING_WINDOW_LEN=100, SLIDING_WINDOW_STEP=50):
    x_sw, y_sw, d_sw = load_domain_data(device, args, SLIDING_WINDOW_LEN, SLIDING_WINDOW_STEP)
    print("x_win_all: {}, y_win_all: {}, d_win_all: {}".format(x_sw.shape, y_sw.shape, d_sw.shape))
    x_train, x_val, x_test, \
    y_train, y_val, y_test, \
    d_train, d_val, d_test = train_test_val_split(x_sw, y_sw, d_sw, split_ratio=args.ratio)
    ...
    print("x_train: {}, x_val: {}, x_test: {}, y_train: {}, y_val: {}, y_test: {}, "
          ...
          .format(x_train.shape, x_val.shape, x_test.shape, y_train.shape, y_val.shape,
                  y_test.shape, d_train.shape, d_val.shape, d_test.shape))

    return x_train, x_val, x_test, y_train, y_val, y_test, d_train, d_val, d_test


def preprocess_hhar_random_fine(device, args, SLIDING_WINDOW_LEN=100, SLIDING_WINDOW_STEP=50):
    x_sw, y_sw, d_sw = load_domain_data(device, args, SLIDING_WINDOW_LEN, SLIDING_WINDOW_STEP)
    x_train, x_val, x_test, \
    y_train, y_val, y_test, \
    d_train, d_val, d_test = train_test_val_fine_split(x_sw, y_sw, d_sw, split_ratio=args.ratio)
...
def prep_hhar(args, SLIDING_WINDOW_LEN=100, SLIDING_WINDOW_STEP=50, device='Phones', train_user=None, test_user=None):
    ...
    if args.cases == 'random':
        x_train, x_val, x_test, y_train, y_val, y_test, d_train, d_val, d_test = \
            split_train_test(device, args, SLIDING_WINDOW_LEN=SLIDING_WINDOW_LEN,
                             SLIDING_WINDOW_STEP=SLIDING_WINDOW_STEP)
        assert x_train.shape[0] == y_train.shape[0] == d_train.shape[0]
    elif args.cases == 'random_fine':
        x_train, x_val, x_test, y_train, y_val, y_test, d_train, d_val, d_test = \
            preprocess_hhar_random_fine(device, args, SLIDING_WINDOW_LEN=SLIDING_WINDOW_LEN,
                                        SLIDING_WINDOW_STEP=SLIDING_WINDOW_STEP)
        assert x_train.shape[0] == y_train.shape[0] == d_train.shape[0]
        ...
        x_train, x_test, y_train, y_test, d_train, d_test = \
            split_train_test_subject(train_user, test_user, device, SLIDING_WINDOW_LEN=SLIDING_WINDOW_LEN,
                                     SLIDING_WINDOW_STEP=SLIDING_WINDOW_STEP)
    elif args.cases == 'cross_device':
        x_train, _, _, y_train, _, _, d_train, _, _ = \
            split_train_test(device, args, SLIDING_WINDOW_LEN=SLIDING_WINDOW_LEN,
                             SLIDING_WINDOW_STEP=SLIDING_WINDOW_STEP)
        test_device = ['Phones', 'Watch']
        test_device.remove(device)
        print('source device:', device, 'target device', test_device[0])
        x_test_1, x_test_2, x_val, y_test_1, y_test_2, y_val, d_test_1, d_test_2, d_val = \
            split_train_test(test_device[0], args, SLIDING_WINDOW_LEN=SLIDING_WINDOW_LEN,
                             SLIDING_WINDOW_STEP=SLIDING_WINDOW_STEP)
        ...
        x_train_1, x_val, x_test, y_train_1, y_val, y_test, d_train_1, d_val, d_test = \
            split_train_test(device, args, SLIDING_WINDOW_LEN=SLIDING_WINDOW_LEN,
                             SLIDING_WINDOW_STEP=SLIDING_WINDOW_STEP)
        test_device = ['Phones', 'Watch']
        test_device.remove(device)
        print('source device:', device, 'target device', test_device[0])
        x_train_2, _, _, y_train_2, _, _, d_train_2, _, _ = \
            split_train_test(test_device[0], args, SLIDING_WINDOW_LEN=SLIDING_WINDOW_LEN,
                             SLIDING_WINDOW_STEP=SLIDING_WINDOW_STEP)
    ...
    if args.cases in ['random', 'random_fine', 'cross_device', 'joint_device']:
