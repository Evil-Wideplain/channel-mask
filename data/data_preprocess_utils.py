...
def train_test_val_fine_split(x_win_all, y_win_all, d_win_all, split_ratio=None):
    """
    :param x_win_all:
    :param y_win_all:
    :param d_win_all:
    :param split_ratio: change type to list, have two numbers, The first number describes how the data will be split
    into training and testing sets, and the second number has the same effect, dividing the previously split training
    set into a validation set, where the validator can be used to fine-tune the model.
愿望清单
购物车
探索
New Featured Carousel
《堡垒之夜》
第 3 章第 4 赛季：PARADISE
将建筑物 Chrome 化，将你自己 Chrome 化，并发挥 Chrome 武器的威力，来拥抱 Chrome 吧。
免费
免费游玩

    第一个数字描述将如何把数据拆分为训练集和测试集，第二个数字有着同样的作用给，将之前划分的训练集中划分出验证集，验证机可用于微调模型.
    :return:
    """
    if split_ratio is None:
        split_ratio = [0.2, 1 / 8]
    split_ratio = split_ratio.split(',')
    first_ratio = float(split_ratio[0])
    second_ratio = float(split_ratio[1])

    # split all data into train and test
    x_win_train, x_win_test, y_win_train, y_win_test, d_win_train, d_win_test = \
        train_test_split(x_win_all, y_win_all, d_win_all, test_size=first_ratio, random_state=0)

    # split train into train and validation with the same ratio
    _, x_win_val, _, y_win_val, _, d_win_val = \
        train_test_split(x_win_train, y_win_train, d_win_train, test_size=second_ratio, random_state=0)
...
def train_test_val_split(x_win_all, y_win_all, d_win_all, split_ratio=None):
    if split_ratio is None:
        split_ratio = [0.2, 1 / 8]
    split_ratio = split_ratio.split(',')
    first_ratio = float(split_ratio[0])
...
def sliding_window(a, ws, ss=None, flatten=True):
    ...
    if np.any(ws > shape):
        raise ValueError('ws cannot be larger than a in any dimension. a.shape was %s and ws was %s'
                         % (str(a.shape), str(ws)))
    # 去掉除0警告
