...
def baseline(args=cl_args, num=1):
    ...
    CL_Base(args, backbones=['DCL', 'FCN'],
            ...
            # 'ucihar', 'shar', 'deviceMotion', 'pamap2', 'WISDM', 'USCHAR', 'DSADS',
            datasets=['hhar'],
            num=num, augs=_aug)
...
def CL_All_mask_rate(args=cl_args, num=1, mask_num=1/6):
    ...
    _aug = [
        ...
        'perm,' + mask_name,
        'scale,' + mask_name]
    # TODO 暂时在这里设定为 1/6 我不大确定在parameter中设定是否准确
    args.mask_num = mask_num
    CL_Base(args, backbones=['FCN'], datasets=['pamap2', 'DSADS', 'deviceMotion'], num=num, augs=_aug)
...
def CL_All_mask_features(args=cl_args, num=1, mask_num=1/6):
    ...
    # TODO 暂时在这里设定为 1/6 我不大确定在parameter中设定是否准确
    args.mask_num = mask_num
    CL_Base(args, backbones=['FCN'], datasets=['ucihar', 'shar', 'deviceMotion', 'pamap2', 'WISDM', 'USCHAR', 'DSADS'], num=num, augs=_aug)
